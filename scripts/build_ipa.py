...
if os.path.isdir(products_apps):
    for f in os.listdir(products_apps):
        print(f"  {f}")

app_names = [f for f in os.listdir(products_apps) if f.endswith('.app')] if os.path.isdir(products_apps) else []
...
print(f"IPA created: {ipa_path} ({os.path.getsize(ipa_path)} bytes)")
