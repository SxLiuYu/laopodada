...
from auto_tag import auto_tag_image, auto_tag_bytes
...
@app.post("/api/v1/items/auto-tag")
def auto_tag_endpoint():
    """AI 自动识别衣服类型 + 颜色 + 季节。
    入参: {image_url: "https://..."} 或 multipart file
    返回: {category, color, season, title, confidence}
    """
    if "file" in request.files:
        f = request.files["file"]
        tag = auto_tag_bytes(f.read(), f.filename or "upload.jpg")
    else:
        data = request.get_json(silent=True) or {}
        url = (data.get("image_url") or "").strip()
        if not url:
            abort(400, description="缺少 file 或 image_url")
        tag = auto_tag_image(url)
    if not tag:
        abort(503, description="AI 识别失败,请手动填写")
    return jsonify(tag)
...
def create_item():
    ...
    auto_tag = (request.form.get("auto_tag") or "").lower() in ("1", "true", "yes")
    ...
    img_bytes = f.read()
    saved = _save_three_sizes(img_bytes)

    # AI 自动识别
    if auto_tag:
        tag = auto_tag_bytes(img_bytes, f.filename or "upload.jpg")
        if tag:
            category = tag["category"] if not raw_category or raw_category == "auto" else raw_category
            color = color or tag.get("color")
            season = season or tag.get("season")
            title = title or tag.get("title")
            print(f"[create_item] auto-tag: {tag}")
...
def update_item(item_id: str):
    ...
    fields = {}
    for key in ("category", "color", "season", "title", "brand", "note"):
        if key in body:
            fields[key] = body[key]
    if not fields:
        abort(400, description="没有需要更新的字段")
    set_clause = ", ".join(f"{k}=?" for k in fields)
    db.execute(f"UPDATE wardrobe_items SET {set_clause} WHERE id=?", list(fields.values()) + [item_id])
    updated = db.execute("SELECT * FROM wardrobe_items WHERE id=?", (item_id,)).fetchone()
    return jsonify(item=_row_to_wardrobe(updated), ok=True)
...
from recommend import OutfitRecommender  # noqa: E402  (intentionally late import after route defs)
...
def generate_health_article():
    ...
    user_msg = f"主题: {topic}" + (f"\n类别: {category}" if category else "")  # noqa: F841  (kept for debugging)
    full_msg = f"{HEALTH_GENERATION_SYSTEM}\n\n{user_msg}\n只返回 JSON,不要其他文字。"  # noqa: F841  (kept for debugging)
...
def create_health_article():
    ...
    Used by scripts/seed_demo.py to demo-seed data. No auth (intentional,
    internal use). Required fields: title, category, content. Optional:
    summary, read_minutes, source.
    """
    import uuid as _uuid
    body = request.get_json(silent=True) or {}
    ...
    article = {
        ...
        "summary": body.get("summary") or (body["content"][:80] + "..."),
